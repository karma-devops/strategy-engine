...
def get_effective_rate(db: Session, balance: float, config: WithdrawalConfig) -> float:
    ...
    # BUG #22: boundary semantics are intentional and tested by adjacency:
    #   - tier N matches  min_N <= balance < max_N  (inclusive min, exclusive max)
    #   - the next tier's min is exactly max_N, so balance == max_N of tier N
    #     correctly rolls into tier N+1 (no gap, no overlap).
    #   e.g. balance == 100000 hits the tier whose min <= 100000 < max, or the
    #   open-ended tier (max is None) when 100000 >= its min.
    for rule in rules:
        max_val = rule["max"]
        if max_val is None:
            if balance >= rule["min"]:
