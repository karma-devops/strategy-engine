...
# Replaced legacy MAKER_FEE/TAKER_FEE with the full X4 ExecutionCostModel
# (fee + slippage + spread) so backtests reflect realistic round-trip cost.
COST_MODEL = get_cost_model()  # uses HL-typical defaults; tune via get_cost_model(...)
MAKER_FEE = float(COST_MODEL.maker_fee)
TAKER_FEE = float(COST_MODEL.taker_fee)


def _entry_cost(position_value: float, is_maker: bool = False) -> float:
    """X4: full entry cost (fee + slippage + spread) in quote units."""
    pv = COST_MODEL.maker_fee if is_maker else COST_MODEL.taker_fee
    total_bps = float(pv) + float(COST_MODEL.slippage_bps) + float(COST_MODEL.spread_bps)
    return float(position_value) * total_bps


def _exit_cost(position_value: float, is_maker: bool = False) -> float:
    """X4: full exit cost (fee + slippage) in quote units."""
    pv = COST_MODEL.maker_fee if is_maker else COST_MODEL.taker_fee
    total_bps = float(pv) + float(COST_MODEL.slippage_bps)
    return float(position_value) * total_bps
    ...
    for tick_price in ticks:
        if position.side == "LONG":
            ...
            if position.stop_loss_price is not None and tick_price <= position.stop_loss_price:
                exit_cost = _exit_cost(position.qty * position.stop_loss_price)
        ...
        for i in range(len(df)):
            ...
            if position is not None and prev_fast_ema is not None and prev_medm_ema is not None:
                ...
                if curr_fast is not None and curr_medm is not None:
                    ...
                    if trend_reversal:
                        exit_cost = _exit_cost(position.qty * close)
                        if position.side == "LONG":
