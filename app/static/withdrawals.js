...
async function fetchAccount() {
    try {
        const res = await fetch(`${API_BASE}/api/v2/account`);
...
async function fetchCalculate() {
    try {
        const res = await fetch(`${API_BASE}/api/v2/withdrawals/calculate`);
...
async function fetchHistory() {
    try {
        const res = await fetch(`${API_BASE}/api/v2/withdrawals/history?limit=20`);
...
async function fetchProjection() {
    try {
        const res = await fetch(`${API_BASE}/api/v2/withdrawals/projection?start_capital=10000`);
...
document.getElementById('btn-50').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/v2/withdrawals/manual/50`, { method: 'POST' });
        const result = await res.json();
...
document.getElementById('btn-all').addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/v2/withdrawals/manual/all`, { method: 'POST' });
        const result = await res.json();
        alert(result.message || 'Done');
        await updateUI();
    } catch (err) {
