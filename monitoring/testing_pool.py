...
def promote_asset(token: str) -> dict[str, Any]:
    return {"ok": True, "message": f"Asset {token} promoted to production candidate. Operator must create instance manually."}


def fail_asset(token: str, reason: str = "") -> dict[str, Any]:
    return {"ok": True, "message": f"Asset {token} marked failed. Reason: {reason or 'No reason provided.'}"}
