...
def compute_instance_metrics(
    ...
    # BUG #21 (documented limitation): instances sharing one wallet write
    # identical whole-account AccountSnapshot values tagged with different
    # instance_ids. Therefore return_pct/drawdown derived from snapshots are
    # NOT truly per-instance unless each instance has distinct credentials.
    # TODO: compute per-instance PnL from Trade rows (sum pnl_usd per
    # instance_id) instead of account-value deltas for accurate fleet metrics.
    snapshots = (
