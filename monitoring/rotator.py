...
def approve_recommendation(db: Session, rec_id: str, approved: bool) -> dict[str, Any] | None:
    ...
    # BUG #12: applying approval actually updates the instance allocation.
    # If approved and action is INCREASE/REDUCE, apply suggested_allocation_pct.
    if approved and row.action in ("INCREASE", "REDUCE") and row.suggested_allocation_pct is not None:
        inst = db.query(Instance).filter(Instance.slug == row.instance_slug).first()
        if inst:
            inst.max_position_pct = row.suggested_allocation_pct / 100.0
            print(f"[ROTATOR] Applied {row.action} → {inst.slug} max_position_pct={inst.max_position_pct:.2f}")
    db.commit()
