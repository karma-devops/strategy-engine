...
| UI-1 | **Saved strategy must show on the front end of the LIVE host** (verified 2026-07-24: `/app/strategies` reads `STRATEGIES` in-memory registry at `app/routes.py:1269`, NOT DB; public host `puls-r-engine.6cdzen.easypanel.host` is a SEPARATE deploy with its own DB/filesystem — local `:8792` test saves never appeared there). | Either (a) deploy the saved `strategies/{slug}/strategy.py` file to the live instance + restart, or (b) make the Strategies list also read DB `Strategy` rows (hybrid) so a `/save` shows without a file deploy. | Live host `/app/strategies` lists a strategy saved via Studio |
| UI-2 | **UI "Save & Activate" end-to-end proof on live host.** Local `/save` (`app/routes.py:1890`) now writes file + live-registers (verified), but the public host path was NOT exercised (separate deploy). | Drive Studio paste→Convert→Save&Activate against the live host; confirm the new strategy appears in `/app/strategies` there. | Browser-verified on live host |
| UI-3 | **Strategies list refresh without full restart.** Registry is populated at import (`STRATEGIES = _discover_subdir_strategies()` at `registry.py:86`); new file needs restart to appear in list (live-register covers API but not the rendered HTML list which re-reads `STRATEGIES` per request — OK — but a brand-new subdir added while running still needs import). | Confirm a freshly-added `strategies/{slug}/` is picked up by the list on next request (dynamic discovery re-scans per import only). If not, add a refresh hook or document restart requirement in UI. | List reflects new slug within 1 request post-save |
