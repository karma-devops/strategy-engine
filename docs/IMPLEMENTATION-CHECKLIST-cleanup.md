...
| UI-1 | Saved strategy must show on the front end of the LIVE host. Public host puls-r-engine.6cdzen.easypanel.host is a SEPARATE deploy with its own DB/filesystem - local 8792 test saves never appeared there. | Deploy the saved strategies/{slug}/strategy.py file to the live instance plus restart (separate deploy - NOT actionable from this local box). | Live host /app/strategies lists a strategy saved via Studio |
| UI-2 | UI Save and Activate end-to-end proof on live host. Local /save writes file plus live-registers (verified), but the public host path was NOT exercised (separate deploy). | Drive Studio paste to Convert to Save and Activate against the live host. | Browser-verified on live host |
| UI-3 | Strategies list refresh without full restart. Registry is populated at import; new subdir needs restart to appear in list. | Confirm a freshly-added strategies/{slug}/ is picked up by the list on next request; if not, document restart requirement. | List reflects new slug within 1 request post-save |
