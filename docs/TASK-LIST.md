...
| T4-5 | **Pulse Graph → zero-centered relative oscillation** (senior brief, `pasted-text 13-17-58` + `13-31-02` + `13-47-30`). Brief's verbatim code block REJECTED — it targets a wrong DOM (destroys `<defs>`/gradient, `clientWidth/Height` px coords vs viewBox 1000×200, `.tf-pill`/`switchPulseTimeframe` duplicate of existing `.chip.pulse-range`/`setPulseRange`, `pulse-delta-pct` ID absent). Corrected plan (operator-approved) keeps real DOM + math: baseline at `centerY=100`, Δ% from window's first point, normalize by `max|dev|` (floored 0.02), green/red hemispheres, mobile delta mirror. | Sub-cent equity ($6.02 vs $6.04) renders as flat line pinned to one edge under absolute scaling. | **STEP 1 DONE 2026-07-19 (commit b02aed8)** — `buildPulse()` rewritten to zero-centered relative deviation (anchor = first point in window; `y = centerY - (dev/maxAbsDev)*AMP`, `AMP=centerY*0.82`); area closes to `centerY` not bottom; dot at last point on the wave; `pulse-change` + `pulse-change-m` show ▲/▼ %; gradient swap on both `#pulseGrad`/`#pulseGradM`. Live-served 200 (HTML/JS serve live; no restart needed — engine-1 LIVE, Python routes untouched). NEXT (Step 2): add dashed zero-center baseline `<line>` in `buildPulse()` (current SVG has none) + verify in browser (console clean, line dances around center, pills recalc). Step 3: commit/push/doc per step. |
