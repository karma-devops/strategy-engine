...
| # | Task | Why / Effort |
|---|------|--------------|
| T0-1 | `core/llm.py` converter prompt → align to real `exit_config` contract (not `metadata`) | Generated strategies trade with NO SL/TP/trail, silently. 30 min |
...
| T0-6 | Create `data/` dir at startup (or `Dockerfile`) | Fresh deploy crashes immediately, reproduced. 10 min |
