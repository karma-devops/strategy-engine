...
| T3-0 | **Cross-user data leak on dashboard** — `get_dashboard_api_key()` was hardcoded to the OPERATOR key, so ANY logged-in user saw operator's portfolio/engines via client-side `/api/v2/summary` (operator's $6.50 bled to `babatest`). Also scoped dashboard server-rendered trades/equity-snapshots to the session user. | **FIXED + live-verified 2026-07-19.** `get_dashboard_api_key(request)` now derives the key from the session cookie (per-user `puls_` key); operator basic-auth → operator key; non-operator → own key (403/empty if none). Dashboard trades/AccountSnapshot/Pulse graph + live-exchange fallback scoped to `user_id`/operator-only. Fresh-DB verified: babatest2 dashboard shows 0 operator engines + own key injected. | DONE |
