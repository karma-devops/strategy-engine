...
| T4-5 | **Pulse Graph → zero-centered relative oscillation** (senior brief, `pasted-text 13-17-58` + `13-31-02` + `13-47-30`). Brief's verbatim code block REJECTED — it targets a wrong DOM (destroys `<defs>`/gradient, `clientWidth/Height` px coords vs viewBox 1000×200, `.tf-pill`/`switchPulseTimeframe` duplicate of existing `.chip.pulse-range`/`setPulseRange`, `pulse-delta-pct` ID absent). Corrected plan (operator-approved) keeps real DOM + math: baseline at `centerY=100`, Δ% from window's first point, normalize by `max|dev|` (floored 0.02), green/red hemispheres, mobile delta mirror. | Sub-cent equity ($6.02 vs $6.04) renders as flat line pinned to one edge under absolute scaling. | **STEP 1 DONE (commit b02aed8)** + **STEP 2 DONE (commit 9d3e9b1)** — `buildPulse()` zero-centered relative deviation (anchor = first point; `y = centerY - (dev/maxAbsDev)*AMP`, `AMP=centerY*0.82`); area closes to `centerY`; dot on wave; `pulse-change`/`pulse-change-m` show ▲/▼ %; gradient swap on `#pulseGrad`+`#pulseGradM`. Step 2 adds dashed `#pulse-baseline` `<line>` at `y=100` (first child of `#pulse-svg`+`#pulse-svg-m`), token stroke `var(--border)`, dash `5,5`. **BROWSER-VERIFIED** (DevTools DOM): baseline is firstChild of both SVGs, `y1=y2=100`, `x2=1000`; `pulse-change` live `▲ 0.54%`. Senior's `.panel-chart-wrapper/#pulse-container` selectors were wrong (no such nodes) → targeted real SVG IDs. T4-5 COMPLETE. |
