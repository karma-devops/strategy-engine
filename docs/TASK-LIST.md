...
| T3-0 | **Cross-user data leak on dashboard** — FINAL ROOT CAUSE found 2026-07-19: `get_dashboard_api_key()` held a **module-level `_DASHBOARD_API_KEY_CACHE`** that cached the OPERATOR key on first call and served it to any user whose session-cookie decode fell through. Plus `landing.html` had a `sessionStorage.pulsr_auth` block that built `//operator:pass@host/app/dashboard` Basic-Auth links, caching operator creds in the browser. | **FIXED + pushed 7f71432.** Cache removed (key derived strictly per-request from session cookie; no session => empty, never operator). Landing Basic-Auth shortcut removed. Prior 02272aa/760031b added per-session key + sw.js v2 + no-store header (defense-in-depth). Verified via real proxy URL: fresh user gets empty key + own empty dashboard; operator gets own engines. | DONE |
